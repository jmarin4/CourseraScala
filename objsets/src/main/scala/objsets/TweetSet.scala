...
abstract class TweetSet {
  ...
  def ascendingByRetweetAcc( tail: TweetList ): TweetList;
...
class Empty extends TweetSet {
  ...
  def ascendingByRetweetAcc( tail: TweetList ): TweetList = tail
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
   ...
   def mostRetweeted: Tweet = {
     ...
     def max( f: Tweet, s: Tweet ) : Tweet = {
       if( f.retweets > s.retweets ) f
       else s
     }
  ...
  def ascendingByRetweet: TweetList = ascendingByRetweetAcc(Nil)

  def ascendingByRetweetAcc( tail: TweetList ): TweetList = {
    val mr = mostRetweeted
    remove(mr).ascendingByRetweetAcc( new Cons( mr, tail ) )
  }
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet =
    TweetReader.allTweets.filter( 
        (t: Tweet )=> (google.exists(( x: String ) => t.text.contains(x) )))
  lazy val appleTweets: TweetSet = 
    TweetReader.allTweets.filter( 
        (t: Tweet )=> (apple.exists(( x: String ) => t.text.contains(x) )))
